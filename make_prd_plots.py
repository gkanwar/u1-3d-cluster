...
def plot_results():
    ...
    for e2,(M_bins,MC_bins,hist) in zip(all_e2, all_bins_hists):
        ...
        for L, M_hist in M_hists:
            if use_kde:
                ...
                ys = M_hist
                al.add_errorbar(
                    ys, xs=xs, ax=ax, color=colors[L], marker='.', capsize=0, label=rf'$L={L}$')
        ...
        if L >= 128:
            ...
            if L == 128:
                fit_handle, = axes_MC[1].plot(
                    fit_e2s, fit_fs, linestyle='--', color='xkcd:magenta', zorder=3)
                fit_label = rf'${A:0.2f} e^{{ -{B:0.2f} / e^2 }}$'

    handles = leg_handles_M
    ...
    for i in range(len(handles)):
        ...
        lines = (line, lines[1], lines[2])
        handles[i] = mcontainer.ErrorbarContainer(lines)
    axes_M[1].axvline(fiducial_e2, color='0.5', linewidth=0.5*paper_plt.pix_to_pt)
    axes_MC[1].axvline(fiducial_e2, color='0.5', linewidth=0.5*paper_plt.pix_to_pt)
    axes_M[1].set_xlabel(r'$e^2$')
    axes_MC[1].set_xlabel(r'$e^2$')
    axes_M[1].set_ylabel(r'$\left<O_{CS}^2\right> / V^2$')
    ...
    fig_M.legend(leg_handles_M, leg_labels_M, **legend_params)
    fig_MC.legend(leg_handles_MC, leg_labels_MC, **legend_params)
